...
while running:
    ...
    myfont = pygame.font.SysFont("monospace", 30, bold=Tr)
    mpcs = []
    
    dir_path = os.path.dirname(os.path.realpath(__file__)) + "/../save.json"
    print(dir_path)
    with open(dir_path) as json_file: 
        mpcs = list(map(lambda x: x['name'], json.load(json_file)))
        
        
    step = height // len(mpcs) 
    for x in range(1, len(mpcs)):
        label = myfont.render(mpcs[x], 20, (232,12,12))
        screen.blit(label,(0,x*step))
