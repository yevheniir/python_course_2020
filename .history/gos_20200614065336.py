...
# print(most_occur) 
# -----------------------------------------------------------------------------------------------------------
people = [{"name": "nanachi", "obl": "черкаська"}, 
          {"name": "nezuko", "obl": "черкаська"}, 
          {"name": "кабан", "obl": "київська"}]

# Ініціалізація змінної з результатом
result = {}

# Ітерації по людях для ініціалізації клічів результата
for x in people:
    # Створення ключа бузуючись на області та присвоєння йому 0
    result[x['obl']] = 0

# Ітерація по людях
for x in people:
    # Додавання людини до відповідного регіону
    result[x['obl']] += 1
